...
def buildAndScoreModel() : Double = {
    ...
    val splits = patients.randomSplit(Array(0.7, 0.3))  
    ...
    val query = s"""SELECT noteevents.subject_id,noteevents.hadm_id,combindString(collect_list(cleanString(regexp_replace(regexp_replace(text,"[^a-zA-Z\\s]",' '),"NEWLINE",' ')))) as Status
        ...
        WHERE storetime is not null
        AND iserror = ''
        AND unix_timestamp(admissions.admittime)+86400 < unix_timestamp(admissions.dischtime)
...
var fullModelCVAUCS:Array[Double] = Array()
fullModelCVAUCS = fullModelCVAUCS :+ buildAndScoreModel()
fullModelCVAUCS = fullModelCVAUCS :+ buildAndScoreModel()
fullModelCVAUCS = fullModelCVAUCS :+ buildAndScoreModel()
fullModelCVAUCS = fullModelCVAUCS :+ buildAndScoreModel()
fullModelCVAUCS = fullModelCVAUCS :+ buildAndScoreModel()
...
fullModelCVAUCS = fullModelCVAUCS :+ buildAndScoreModel()
fullModelCVAUCS = fullModelCVAUCS :+ buildAndScoreModel()
fullModelCVAUCS.foreach(println)
